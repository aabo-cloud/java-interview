...
2. 客户端发送 SQL语句，如果是 select语句 会先去查询缓存，如果命中缓存，直接返回结果。MySQL 8.0 后取消缓存。如果表频繁更新，缓存更本就用不上。
...
同时，虽然索引大大提高了查询速度，同时却会降低更新表的速度，如对表进行 INSERT、UPDATE和DELETE。因为更新表时，MySQL不仅要保存数据还要更新索引。建立索引会占用磁盘空间。一般情 况这个问题不太严重，但如果你在一个大表上创建了多种组合索引，索引文件的会膨胀很快。
...
MySQL server层 会调用 innodb引擎 的接口，去获取 0 到 一千万零一百的区间的完整数据。数据返回给 server层 之后根据 offset 的值最后只留下最后面的 100条，放到 server层 的结果集中，返回给客户端。
...
```SQL
select id from page order by id limit 10000000, 1;
```
...
```SQL
select * from page where id >= (select id from page order by id limit 10000000, 1) order by id limit 100;
```
...
当不确定某个查询语句的效率如何时，或者该查询语句较慢，考虑如何优化时。可以使用 explain 关键字查看该查询语句的执行计划分析。主要看该查询语句分解之后的子语句对表的访问方式，有没有走全文查找、全文索引之类的。分析要不要考虑加索引进行优化。
