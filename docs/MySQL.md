...
最早的数据库系统，只有读读之间可以并发，读写，写读，写写都要阻塞。引⼊多版本之 后，只有写写之间相互阻塞，其他三种操作都可以并⾏，这样⼤幅度提⾼了InnoDB的并发 度。
...
同时还可以解决脏读，幻读，不可重复读等事务隔离问题，但不能解决更新丢失问题 MVCC只在读取已提交和可重复 读两种隔离级别下有作⽤。

MVCC常见的实现⽅式乐观锁和悲观锁

MVCC是⾏级锁的变种，很多情况下避免了加锁操作。 应对⾼并发事务, MVCC⽐单纯的加锁更⾼效;
